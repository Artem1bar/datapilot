...
- [ ] **Structured outputs**: move plan generation and the verification agent from free-text JSON scraping (`_extract_json_from_response`) to `client.messages.parse()` / `output_config.format` with a `CleaningPlan` JSON schema, including per-step `rationale` and model-reported `confidence` (frontend currently fakes 0.9).
- [ ] **Remediation-loop convergence** (`cleaning_task.py`): validate agent remediation steps before executing; treat flags that survive a round as "unresolvable" instead of re-burning rounds; raise the verification agent's 4096-token output cap; final report must always reflect post-remediation state.
...
- [ ] **Plan approval gate in chat**: pause `runCleaningWorkflow` at the plan card with per-step toggle/edit + explicit "Apply N steps" (reuse Clean.tsx checkbox pattern). Surface `steps_failed` and remediation rounds in the results card.
