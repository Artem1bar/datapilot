...
- **Statistical analysis** — ask analytical questions and DataPilot executes them against your full dataset using pandas/scipy/statsmodels: 33 operations spanning aggregation, correlation, hypothesis tests (t-test, ANOVA with Tukey HSD, Kruskal-Wallis, Mann-Whitney, Wilcoxon, chi-square, proportion tests), regression (OLS with robust SEs, logistic, Poisson/negative binomial, quantile), time series (STL decomposition, ADF/KPSS, ACF/PACF, ARIMA forecasts, Granger), and weighted survey estimation. Claude chooses the test and explains the result, but never supplies a number — figures are measured, not generated
- **Statistics you can defend** — every test reports an effect size, a confidence interval, and explicit assumption checks (Levene's, Shapiro-Wilk, expected cell counts) alongside the p-value, and p-values are adjusted for the number of tests run. A violated assumption is stated with the finding, not buried under it
- **Survey weights done properly** — when a dataset carries sampling weights, unweighted means are wrong in a measurable way. DataPilot detects the weight column, estimates with design-based standard errors, applies the Rao-Scott correction to weighted crosstabs, reports the design effect and effective sample size, and treats a subgroup as a domain rather than a filter
- **Code export** — every answer ships the Python and R that reproduces it, so a researcher can rerun the analysis in their own environment and check the number. Tests execute the generated Python and compare it against the pipeline's own output
- **Methods note** — every answer carries a record of what ran: operations and parameters, n and excluded rows per operation, assumption verdicts, and exact library versions, rendered as a copyable methods note. Nothing in that record is generated by a model
- **Data visualizations** — computed results are rendered as bar, line, scatter, and distribution charts in a slide-out panel
- **Scatter and bubble plotter** — pick any two numeric columns, optionally a column to color by and a numeric column to size the points, from the attach menu or the Visualizations panel. Every row is plotted on numeric axes with a least-squares line fitted server-side on all complete rows (large datasets are sampled to 2,000 points with a fixed seed for display, and the chart says so). The plot is recorded in the conversation with the same methods note and Python/R export as an analysis answer, and no AI call is involved
- **Help reading the plot** — every scatter or bubble plot comes with a reading computed from its statistics: which way the association runs and how strong it is by conventional bands, what the slope means in the data's own units, whether it is distinguishable from zero, and the caveats that apply to that plot (small sample, excluded rows, a pooled line over groups, bubble size). It ends with follow-up questions that one click puts into the chat
...
| `test_analysis_spec.py` | Analysis spec validation — operation whitelist, hallucinated columns, numeric-agg/dtype mismatch, conditional test parameters, category-value checks, chart index range, refusal as a valid outcome |
...
| `test_analysis_inference.py` | Hypothesis tests — t/F/U/W/chi-square/z statistics, effect sizes and intervals against closed-form values, assumption checks, refusal on degenerate input |
