import { Sparkles, BarChart3, FileText, FlaskConical } from "lucide-react";
...
interface EmptyStateProps {
  ...
  /** Loads the bundled demo dataset so first-run users can try the pipeline
   *  without hunting for a CSV of their own. */
  onTrySample?: () => void;
}
...
export function EmptyState({ onChipClick, onTrySample }: EmptyStateProps) {
  return (
        ...
        <div className="mt-8 grid gap-3">
          ...
          {onTrySample && (
            <motion.button
              ...
              type="button"
              onClick={onTrySample}
              className="flex items-center gap-4 rounded-xl border border-dashed border-[var(--line)] bg-transparent p-4 text-left transition-colors duration-200 hover:border-gold-500 hover:bg-brand-50 hover:shadow-sm"
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gold-100 text-gold-600">
                <FlaskConical className="h-5 w-5" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-[13px] font-medium text-ink">Try with sample data</p>
                <p className="mt-0.5 text-[12px] text-ink-muted">
                  Load a small messy sales file and see the whole pipeline in action
                </p>
              </div>
            </motion.button>
          )}
