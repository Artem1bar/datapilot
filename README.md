...
- **Statistical analysis** — ask analytical questions and DataPilot executes them against your full dataset using scipy/pandas: 19 operations spanning aggregation, correlation, and hypothesis tests (t-test, ANOVA with Tukey HSD, Kruskal-Wallis, Mann-Whitney, Wilcoxon, chi-square, proportion tests). Claude chooses the test and explains the result, but never supplies a number — figures are measured, not generated
...
**1,014 tests total** (875 backend + 139 frontend), plus 3 Playwright E2E specs that drive the full stack (stubbed Anthropic) in CI.

### Backend (875 tests) — run from `apps/api/`
...
### Frontend (139 tests) — run from `apps/web/`
